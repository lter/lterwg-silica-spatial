...
status_manifest <- system2(
  "Rscript",
  c("tools/build_targeted_rerun_manifest.R", "--outdir", pre_aurora_root(review_root))
)

if (!identical(status_manifest, 0L)) {
  stop("Pre-Aurora review stopped while building the strict HydroSHEDS candidate list.", call. = FALSE)
}

strict_candidates_path <- latest_pre_aurora_file(
  file.path(pre_aurora_root(review_root), "rerun"),
  "targeted_rerun_hydrosheds_candidates_for_approval_*.csv"
)

if (is.na(strict_candidates_path) || !file.exists(strict_candidates_path)) {
  stop("Could not find the strict HydroSHEDS candidate file after running the manifest builder.", call. = FALSE)
}
...
ref_path <- resolve_input_path(
  ...
  candidates = c(
    ...
    file.path(default_root, "master", "Site_Reference_Table - WRTDS_Reference_Table_LTER_V2.csv")
  ),
  label = "reference"
)
...
current_path <- resolve_input_path(
  ...
  candidates = c(
    ...
    file.path(default_root, "si-extracted-data", "all_data_extractions", "all-data_si-extract_3_20260509_recombinedLocal_obidosDedup.csv")
  ),
  label = "combined"
)
legacy_path <- resolve_input_path(
  ...
  candidates = c(
    file.path(default_root, "si-extracted-data", "all_data_extractions", "all-data_si-extract_2_20250325.csv"),
...
approval_path <- file.path(
  ...
  paste0("pre_aurora_approval_mixed_", today_tag, ".csv")
)
holdout_path <- file.path(
  pre_aurora_approvals_dir(review_root),
  paste0("pre_aurora_holdouts_review_", today_tag, ".csv")
)
...
summary_path <- file.path(
  ...
  paste0("pre_aurora_review_summary_", today_tag, ".csv")
)
...
message("Step 3 of 4: wrote the mixed pre-Aurora approval file")
...
message("  Rscript 01_pre_aurora_run/02_split_approved_runs.R")
