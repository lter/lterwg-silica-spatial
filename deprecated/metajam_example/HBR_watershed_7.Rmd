...
In our case the link is: https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-hbr.9.15&entityid=3ccd9dee81ff38e5bd15f2f69dc5caa2
...
my_data_url <- "https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-hbr.9.15&entityid=3ccd9dee81ff38e5bd15f2f69dc5caa2"
...
HBR_watershed_7_data <- my_data$data
...
# the tidy way
HBR_watershed_7_data <- HBR_watershed_7_data %>% 
...
HBR_watershed_7_data <- HBR_watershed_7_data %>% 
  add_column('Site/Stream Name' = 'knb-lter-hbr.9.15', .before = "Year")
...
names(HBR_watershed_7_data)[7:31] <- substring(names(HBR_watershed_7_data)[7:31],7,15)
...
(fuzzy_match <- tibble(template = names(template)) %>%
   mutate(watershed7 = names(HBR_watershed_7_data)[amatch(tolower(template), tolower(names(HBR_watershed_7_data)), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)


(lookup_table <- fuzzy_match %>%
  mutate(watershed7 = case_when(
...
lookup_table <- lookup_table %>%
  filter(!is.na(watershed7))

correct_colnames <- lookup_table$template

new_datatable7 <- HBR_watershed_7_data %>% 
...
(fuzzy_match <- tibble(solute_units_Template$Measurement) %>%
   mutate(watershed_units_7 = attributes$attributeName[amatch(tolower(solute_units_Template$Measurement), tolower(attributes$attributeName), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)

fuzzy_match$watershed_units_7 <- gsub("SiO2", "Si", fuzzy_match$watershed_units_7)
...
sum(is.na(new_datatable7))/prod(dim(new_datatable7))

#for columns
colMeans(is.na(new_datatable7))
