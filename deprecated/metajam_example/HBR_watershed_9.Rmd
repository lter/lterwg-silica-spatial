...
In our case the link is: https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-hbr.11.16&entityid=5d79a0ab7b312e7f1aa7b4cd1e64ac78
...
my_data_url <- "https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-hbr.11.16&entityid=5d79a0ab7b312e7f1aa7b4cd1e64ac78"
...
HBR_watershed_9_data <- my_data$data
...
# the tidy way
HBR_watershed_9_data <- HBR_watershed_9_data %>% 
...
HBR_watershed_9_data <- HBR_watershed_9_data %>% 
  add_column('Site/Stream Name' = 'knb-lter-hbr.11.16', .before = "Year")
...
names(HBR_watershed_9_data)[7:31] <- substring(names(HBR_watershed_9_data)[7:31],7,15)
...
(fuzzy_match <- tibble(template = names(template)) %>%
   mutate(watershed9 = names(HBR_watershed_9_data)[amatch(tolower(template), tolower(names(HBR_watershed_9_data)), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)


(lookup_table <- fuzzy_match %>%
  mutate(watershed9 = case_when(
...
lookup_table <- lookup_table %>%
  filter(!is.na(watershed9))

correct_colnames <- lookup_table$template

new_datatable9 <- HBR_watershed_9_data %>% 
...
(fuzzy_match <- tibble(solute_units_Template$Measurement) %>%
   mutate(watershed_units_9 = attributes$attributeName[amatch(tolower(solute_units_Template$Measurement), tolower(attributes$attributeName), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)

fuzzy_match$watershed_units_9 <- gsub("SiO2", "Si", fuzzy_match$watershed_units_9)
...
sum(is.na(new_datatable9))/prod(dim(new_datatable9))

#for columns
colMeans(is.na(new_datatable9))
