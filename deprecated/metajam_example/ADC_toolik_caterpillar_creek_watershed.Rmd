---
title: "ADC_toolik_caterpilar_creek_watershed"
...
Toolik_caterpillar_creek_watershed <- spread(Toolik_caterpillar_creek_watershed, key = 'Type', value = 'Value')

#Unsure what the 'Site/Stream Name' should be so combining river name and station
Toolik_caterpillar_creek_watershed <- Toolik_caterpillar_creek_watershed  %>% 
add_column('Site/Stream Name' = paste(Toolik_caterpillar_creek_watershed$River, Toolik_caterpillar_creek_watershed$Station), .before = 'River')
...
(fuzzy_match <- tibble(template = names(template)) %>%
   mutate(watershed_toolik_3 = names(Toolik_caterpillar_creek_watershed)[amatch(tolower(template), tolower(names(Toolik_caterpillar_creek_watershed)), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)
...
(lookup_table <- fuzzy_match %>%
  mutate(watershed_toolik_3 = case_when(
    template %in% c("TN", "TP", "TOC", "DIC", "PO4") ~ NA_character_,
    TRUE ~ watershed_toolik_3)))

lookup_table <- lookup_table %>%
  filter(!is.na(watershed_toolik_3))
...
new_datatable_toolik_3 <- Toolik_caterpillar_creek_watershed %>% 
...
writeDataTable(wb = wb, sheet = 1, x = new_datatable_toolik_3)

addWorksheet(wb = wb, sheetName = "Solute Units (Sheet 2)", gridLines = FALSE)

writeData(wb = wb, sheet = 2, x = solute_units_table_new)

saveWorkbook(wb, "ADC_Toolik_caterpillar_creek_watershed.xlsx", overwrite = TRUE)
