---
title: "NWT_martinelli_watershed"
...
In our case the link is: https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-nwt.112.2&entityid=5852142eeaf3bcc68d6936e0f145cc0c
...
my_data_url <- "https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-nwt.112.2&entityid=5852142eeaf3bcc68d6936e0f145cc0c"
...
martinelli_NWT_data <- my_data$data
...
martinelli_NWT_data <- martinelli_NWT_data %>% 
...
names(martinelli_NWT_data) <- gsub("[^0-9A-Za-z///' ]","", names(martinelli_NWT_data))
...
(fuzzy_match <- tibble(template = names(template)) %>%
   mutate(watershed12 = names(martinelli_NWT_data)[amatch(tolower(template), tolower(names(martinelli_NWT_data)), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)
...
(lookup_table <- fuzzy_match %>%
  mutate(watershed12 = case_when(
    template %in% c("DIN", "NOx", "DIC") ~ NA_character_,
    TRUE ~ watershed12)))

lookup_table <- lookup_table %>%
  filter(!is.na(watershed12))
...
new_datatable_NWT_3 <- martinelli_NWT_data %>% 
...
#new_datatable_NWT_3 <- new_datatable_NWT_3 %>% 
...
(fuzzy_match <- tibble(solute_units_Template$Measurement) %>%
   mutate(watershed_units_12 = attributes$attributeName[amatch(tolower(solute_units_Template$Measurement), tolower(attributes$attributeName), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)
...
(lookup_table_units <- fuzzy_match %>%
  mutate(watershed_units_12 = case_when(
    solute_units_Template$Measurement %in% c("DIC","DO", "DO%", "TKN", "PON") ~ NA_character_, TRUE ~ watershed_units_12)))
...
sum(is.na(new_datatable_NWT_3))/prod(dim(new_datatable_NWT_3))

#for columns
colMeans(is.na(new_datatable_NWT_3))
