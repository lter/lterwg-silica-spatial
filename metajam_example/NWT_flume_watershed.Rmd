...
In our case the link is: https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-nwt.162.1&entityid=8f10ecbc48e8432a7100fa07f80f34bb
...
my_data_url <- "https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-nwt.162.1&entityid=8f10ecbc48e8432a7100fa07f80f34bb"
...
my_data <- read_d1_files(downloaded_data, na = c("NP", "Not Performed"))

flume_NWT_data <- my_data$data
...
names(flume_NWT_data) <- gsub("[^0-9A-Za-z///' ]","", names(flume_NWT_data))
...
(fuzzy_match <- tibble(template = names(template)) %>%
   mutate(watershed14 = names(flume_NWT_data)[amatch(tolower(template), tolower(names(flume_NWT_data)), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)
...
(lookup_table <- fuzzy_match %>%
  mutate(watershed14 = case_when(
    template %in% c("DIN", "NOx", "DIC") ~ NA_character_,
    TRUE ~ watershed14)))

lookup_table <- lookup_table %>%
  filter(!is.na(watershed14))
...
new_datatable_NWT_5 <- flume_NWT_data %>% 
...
(fuzzy_match <- tibble(solute_units_Template$Measurement) %>%
   mutate(watershed_units_14 = attributes$attributeName[amatch(tolower(solute_units_Template$Measurement), tolower(attributes$attributeName), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)
...
(lookup_table_units <- fuzzy_match %>%
  mutate(watershed_units_14 = case_when(
    solute_units_Template$Measurement %in% c("DIC","DO", "DO%", "TKN", "PON") ~ NA_character_, TRUE ~ watershed_units_14)))
...
sum(is.na(new_datatable_NWT_5))/prod(dim(new_datatable_NWT_5))

#for columns
colMeans(is.na(new_datatable_NWT_5))
