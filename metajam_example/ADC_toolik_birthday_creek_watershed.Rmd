...
#Making watershed specific data sets
Toolik_birthday_creek_watershed <- Toolik_watershed_data %>% 
  filter(River == "Birthday Creek")

Toolik_burn_reference_watershed <- Toolik_watershed_data %>% 
  filter(River == "Burn Reference Streams")

Toolik_caterpillar_creek_watershed <- Toolik_watershed_data %>% 
  filter(River == "Caterpillar Creek")
...
Toolik_dimple_outlet_watershed <- Toolik_watershed_data %>% 
  filter(River == "Dimple Outlet")
...
Toolik_birthday_creek_watershed <- spread(Toolik_birthday_creek_watershed , key = 'Type', value = 'Value')

#Removing columns we don't need
Toolik_birthday_creek_watershed <- Toolik_birthday_creek_watershed %>% 
  select(-'Site Code', -'Station', -'Reach', -'Method / Instrument', -'Units', -'Comments')  %>% 
  rename('Sampling Date' = 'Date',  'Site/Stream Name' = 'River')


#extracting 'time' from sampling date column and making it it's own column
Toolik_birthday_creek_watershed <- Toolik_birthday_creek_watershed %>% 
  add_column('Time' = substr(Toolik_birthday_creek_watershed$`Sampling Date`, 11, 16), .after = 'Sampling Date')
#removing empty time sot "00:00" from sampling date column since it is "00:00" in every cell (not informative)
Toolik_birthday_creek_watershed$`Sampling Date` <- gsub("00:00", "", Toolik_birthday_creek_watershed$`Sampling Date`)

#Renaming columns to match template abbreviations
Toolik_birthday_creek_watershed <- Toolik_birthday_creek_watershed %>% 
  rename('Ca' = 'Calcium', 'alkalinity' = 'Alkalinity', 'DSi' = 'Silica', 'Na' = 'Sodium', 'K' = 'Potassium', 'Mg' = 'Magnesium', 'CO2' = 'Carbon Dioxide', 'TDP' = 'Total Dissolved Phosphorus', 'DOC' = 'Dissolved Organic Carbon', 'CH4' = 'Methane', 'NH4' = 'Ammonium', 'TDN' = 'Total Dissolved Nitrogen', 'Suspended Chl' = 'Sestonic Chlorophyll a')
...
(fuzzy_match <- tibble(template = names(template)) %>%
   mutate(watershed_toolik_1 = names(Toolik_birthday_creek_watershed)[amatch(tolower(template), tolower(names(Toolik_birthday_creek_watershed )), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)
...
(lookup_table <- fuzzy_match %>%
  mutate(watershed_toolik_1 = case_when(
    template %in% c("TN", "TP", "TOC", "DIC", "Cl") ~ NA_character_,
    TRUE ~ watershed_toolik_1)))

lookup_table <- lookup_table %>%
  filter(!is.na(watershed_toolik_1))
...
new_datatable_toolik_1 <- Toolik_birthday_creek_watershed %>% 
...
(lookup_table_units <- fuzzy_match %>%
  mutate(attributes = case_when(
    solute_units_Template$Measurement %in% c("Q", "DIC","DO", "DO%", "TKN", "PON", "TOC", "TN", "DON", "TP", "DOP", "Cl", "PP") ~ NA_character_, TRUE ~ watershed_units)))
...
writeDataTable(wb = wb, sheet = 1, x = new_datatable_toolik_1)

addWorksheet(wb = wb, sheetName = "Solute Units (Sheet 2)", gridLines = FALSE)

writeData(wb = wb, sheet = 2, x = solute_units_table_new)

saveWorkbook(wb, "Toolik_birthday_creek_watershed.xlsx", overwrite = TRUE)
...
sum(is.na(new_datatable_toolik_1))/prod(dim(new_datatable_toolik_1))

#for columns
colMeans(is.na(new_datatable_toolik_1))
