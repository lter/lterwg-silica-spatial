...
downloaded_data <- download_d1_data("https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-hbr.7.15&entityid=255fa6b525fed95cdc1931a355277b21", path = desired_path_to_data)
...
HBR_watershed_5_data <- cbind(HBR_watershed_5_data, 'LTER' = LTER_name)

#There is no site name in the csv file so I pulled this site identifier from the data portal

Site_name <- rep('knb-lter-hbr.7.15', 652)

HBR_watershed_5_data <- cbind(HBR_watershed_5_data, 'Site/Stream Name' = Site_name)
...
names(HBR_watershed_5_data)[5:29] <- substring(names(HBR_watershed_5_data)[5:29],7,15)
