...
In our case the link is: https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-hbr.8.15&entityid=6b6d95b59d113b9c888078aff67efdb0
...
my_data_url <- "https://portal.edirepository.org/nis/dataviewer?packageid=knb-lter-hbr.8.15&entityid=6b6d95b59d113b9c888078aff67efdb0"
...
HBR_watershed_6_data <- my_data$data
...
# the tidy way
HBR_watershed_6_data <- HBR_watershed_6_data %>% 
...
HBR_watershed_6_data <- HBR_watershed_6_data %>% 
  add_column('Site/Stream Name' = 'knb-lter-hbr.8.15', .before = "Year")
...
names(HBR_watershed_6_data)[7:31] <- substring(names(HBR_watershed_6_data)[7:31],7,15)
...
(fuzzy_match <- tibble(template = names(template)) %>%
   mutate(watershed6 = names(HBR_watershed_6_data)[amatch(tolower(template), tolower(names(HBR_watershed_6_data)), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)


(lookup_table <- fuzzy_match %>%
  mutate(watershed6 = case_when(
...
lookup_table <- lookup_table %>%
  filter(!is.na(watershed6))

correct_colnames <- lookup_table$template

new_datatable6 <- HBR_watershed_6_data %>% 
...
(fuzzy_match <- tibble(solute_units_Template$Measurement) %>%
   mutate(watershed_units_6 = attributes$attributeName[amatch(tolower(solute_units_Template$Measurement), tolower(attributes$attributeName), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)

fuzzy_match$watershed_units_6 <- gsub("SiO2", "Si", fuzzy_match$watershed_units_6)
...
sum(is.na(new_datatable6))/prod(dim(new_datatable6))

#for columns
colMeans(is.na(new_datatable6))
