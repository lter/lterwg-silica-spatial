...
#I want to make an excel file about all of the columns that are close matches but not quite the same to ask the PIs about

#my_data$attribute_metadata

close_matches_arikaree_NWT_data <- my_data$attribute_metadata %>% 
  filter(attributeName %in% c('NO3-','PO4---','NH4+','Ca++','SO4--','CI-','Na+','K+','Mg++'))

write.csv(close_matches_arikaree_NWT_data, file = "close_matches_arikaree_NWT_data.csv", row.names = FALSE)
