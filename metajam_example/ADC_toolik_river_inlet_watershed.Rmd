...
Toolik_river_inlet_watershed <- Toolik_watershed_data %>% 
  filter(River == "Toolik Inlet")


Toolik_river_inlet_watershed  <- Toolik_river_inlet_watershed  %>% 
add_column('Site/Stream Name' = paste(Toolik_river_inlet_watershed$River, Toolik_river_inlet_watershed$Station), .before = 'River')

#Toolik_rivers_watershed <- Toolik_watershed_data %>% 
 # filter(River %in% c( "I-Minus 2", "I-Minus 2 TK", "Imnavait Creek", "Itkillik Tributary", "Kuparuk River", "North River", "Oksrukuyik Creek", "Roche Moutonnee Creek", "Shrew River", "South River", "South River Trib", "Toolik Inlet","Toolik River", "Toolik River TK", "Toolik River TK-4", "Trevor Creek", "Upper Kuparuk", "VTK IMP", "VTK REF"))
...
Toolik_river_inlet_watershed<- spread(Toolik_river_inlet_watershed, key = 'Type', value = 'Value')
...
Toolik_river_inlet_watershed <- Toolik_river_inlet_watershed %>% 
  add_column('Time' = substr(Toolik_river_inlet_watershed$`Sampling Date`, 11, 16), .after = 'Sampling Date')

#removing time from date column
Toolik_river_inlet_watershed <- Toolik_river_inlet_watershed %>% 
  mutate('Sampling Date' = substr(Toolik_river_inlet_watershed$`Sampling Date`, 1, 10))


#Renaming columns to match template abbreviations
Toolik_river_inlet_watershed  <- Toolik_river_inlet_watershed %>% 
  rename('Spec Cond' = 'Specific Conductivity', 'TDP' = 'Total Dissolved Phosphorus', 'DOC' = 'Dissolved Organic Carbon', 'NH4' = 'Ammonium', 'TDN' = 'Total Dissolved Nitrogen', 'Temp C' = 'Temp when sample collected', 'SRP' = 'Soluble Reactive Phosphorus', 'NOx' = 'NitrateNitrite')
...
(fuzzy_match <- tibble(template = names(template)) %>%
   mutate(watershed_toolik_5 = names(Toolik_river_inlet_watershed)[amatch(tolower(template), tolower(names(Toolik_river_inlet_watershed)), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)
...
(lookup_table <- fuzzy_match %>%
  mutate(watershed_toolik_5 = case_when(
    template %in% c("TN", "TP", "TOC", "DIC") ~ NA_character_,
    TRUE ~ watershed_toolik_5)))

lookup_table <- lookup_table %>%
  filter(!is.na(watershed_toolik_5))
...
new_datatable_toolik_5 <- Toolik_river_inlet_watershed %>% 
...
attributes$Measurement <- gsub('Phosphate', 'PO4', attributes$Measurement)

attributes$Measurement <- gsub('NitrateNitrite', 'NOx', attributes$Measurement)
...
writeDataTable(wb = wb, sheet = 1, x = new_datatable_toolik_5)

addWorksheet(wb = wb, sheetName = "Solute Units (Sheet 2)", gridLines = FALSE)

writeData(wb = wb, sheet = 2, x = solute_units_table_new)

saveWorkbook(wb, "ADC_Toolik_river_inlet_watershed.xlsx", overwrite = TRUE)
