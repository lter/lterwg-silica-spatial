...
(fuzzy_match <- tibble(solute_units_Template$Measurement) %>%
   mutate(watershed_units_8 = attributes$attributeName[amatch(tolower(solute_units_Template$Measurement), tolower(attributes$attributeName), maxDist = 1, weight = c(d=1,i=0.1,s=1,t=1))])
)

fuzzy_match$watershed_units_8 <- gsub("SiO2", "Si", fuzzy_match$watershed_units_8)
...
sum(is.na(new_datatable8))/prod(dim(new_datatable8))

#for columns
colMeans(is.na(new_datatable8))
