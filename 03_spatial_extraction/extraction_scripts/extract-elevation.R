...
load_workflow_packages(c("tidyverse", "sf", "stars", "terra", "exactextractr", "starsExtra", "googledrive", "readxl", "spatialEco"))
...
# Read in dataset
elev_raw <- terra::rast(x = file.path(raw_driver_dir, "raw-elevation",
                                      "wc2.1_30s_elev.tif"))
...
elev_out_file <- silica_driver_output_file(path, "si-extract_elevation")

# Export the summarized elevation and slope data
write_subset_csv(
  df = elev_export,
  output_path = elev_out_file,
...
googledrive::drive_upload(media = elev_out_file,
                          overwrite = T,
