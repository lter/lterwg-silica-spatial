...
for(region in region_set){
  ...
  region_files <- dir(path = file.path(
    raw_driver_dir, "raw-npp-v061", region
  ))
  file_df <- data.frame(
...
for (a_year in unique(file_set$year)){
  ...
  for (i in 1:nrow(one_year_data)){
      # Read in the raster
      npp_raster <- terra::rast(file.path(raw_driver_dir, "raw-npp-v061", one_year_data$region[i], one_year_data$files[i]))
...
npp_export <- join_extracted_driver_values(sheds, out_df) %>%
  # Remove geometry before writing the table
  sf::st_drop_geometry()  

dplyr::glimpse(npp_export)

npp_out_file <- silica_driver_output_file(path, "si-extract_npp_v061")

# Export the summarized npp data
write_subset_csv(
  df = npp_export,
  output_path = npp_out_file,
