...
precip_files <- dir(path = file.path(raw_driver_dir, "raw-gpcp-precip"))
...
for(k in 1:file_ct){
  ...
  prec_rast <- terra::rast(x = file.path(raw_driver_dir, "raw-gpcp-precip",
                                         focal_precip))
...
month_df <- full_out_df %>%
  ...
  dplyr::summarize(value = mean(avg_mm_precip_per_day, na.rm = TRUE), .groups = "drop") %>%
  ...
  dplyr::mutate(
    ...
    name = paste0("precip_", month_label, "_mm_per_day")
  ) %>%
