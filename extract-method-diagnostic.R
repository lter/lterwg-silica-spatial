...
data_rast_4326 <- terra::rast("extracted-data/raw-lithology-data/glim_wgs84_0point5deg.txt.asc")

# Check CRS
crs(data_rast_4326)

# Re-project into a different CRS to check it out
## NOTE: will do this once but save the product, then comment out the workflow and just read in the different CRS version
# data_rast_5070 <- terra::project(data_rast_4326, y = 'epsg:5070')
# terra::writeRaster(data_rast_5070, "extracted-data/raw-lithology-data/lithology_epsg5070_raster.tiff")
...
data_df_5070 <- data_rast_5070 %>%
  ...
  # Get column names to match
  rename(uniqueID = ID) %>%
  # The conversion subtly changed the integer codes for land areas so fix them
  mutate(value = as.integer(gsub("\\.", "", str_sub(glim_wgs84_0point5deg.txt, 1, 2)))) %>%
...
data_both_crs <- data_df_4326 %>%
  ...
  # Identify differences
  mutate(`4326_vs_5070_diff` = num_pixels_4326 - num_pixels_5070)
